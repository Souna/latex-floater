...
- **The grapher does one curve, no sliders, no inequalities, no parametric/polar.** Only `x` and `y` are variables; any other letter is an error rather than a Desmos-style slider. Self-intersections and extrema of implicit curves aren't detected.
...
5. **Grapher follow-ups.** The panel height is fixed at 240px (`--graph-height`); a drag handle to resize it would be natural. Implicit curves only get axis-intercept points — extrema and self-intersections (the user asked about self-intersections) would need a curve-tracing pass over the marching-squares segments. Parameter sliders (any free letter becomes a slider, as in Desmos) are the obvious next step if people want to explore families of curves. Non-uniform zoom (stretching one axis) isn't supported.
...
- **Graph is blank after opening**: the canvas sizes itself from a ResizeObserver on its wrapper; if the panel was un-hidden without `Grapher.refresh()` the size is still 0. `setGraphOpen()` calls it.
